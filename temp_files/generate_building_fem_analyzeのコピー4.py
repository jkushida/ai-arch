...
MATERIAL_PROPERTIES = {
    ...
    'wood': {
        'density': 500,       # kg/m³
        'E_modulus': 10000,   # MPa  
        'cost_per_m3': 50000, # 円/m³（材料費のみ）
        'co2_per_m3': -800,   # kg-CO2/m³（炭素固定効果）
        'name_ja': '木材',
        # 木材使用時の断面補正係数
        'section_factor': {
            'column': 1.5,    # 柱は1.5倍
...
def create_realistic_building_model(
    ...
    # 材料選択に応じた断面調整
    # 床スラブ厚
    if material_floor1 == 1 or material_floor2 == 1:  # いずれかが木材
        tf_mm_concrete = int(tf)
        tf_mm_wood = int(tf * MATERIAL_PROPERTIES['wood']['section_factor']['slab'])
    else:
        tf_mm_concrete = int(tf)
        tf_mm_wood = int(tf)
    
    # 屋根スラブ厚
    if material_roof == 1:  # 木材
        tr_mm = int(tr * MATERIAL_PROPERTIES['wood']['section_factor']['slab'])
    else:
        tr_mm = int(tr)
    
    # 柱断面
    if material_columns == 1:  # 木材
        bc_mm = int(bc * MATERIAL_PROPERTIES['wood']['section_factor']['column'])
        hc_mm = int(hc * MATERIAL_PROPERTIES['wood']['section_factor']['column'])
    else:
        bc_mm = int(bc)
        hc_mm = int(hc)
    
    # 壁厚
    if material_walls == 1:  # 木材
        tw_ext_mm = int(tw_ext * MATERIAL_PROPERTIES['wood']['section_factor']['wall'])
    else:
...
#        walls.append(stair_wall_1f)
        ...
        if abs(wall_tilt_angle) > 0.1:  # 傾斜がある場合
            if wall_tilt_angle > 0:  # 外傾斜の場合
                # 五角形の断面を持つ壁（上部が屋根を貫通）
                points = [
                    App.Vector(Lx_mm, 0, H1_mm + tf_mm_floor2),                    # 左下
                    App.Vector(Lx_mm + tw_ext_mm, 0, H1_mm + tf_mm_floor2),       # 右下
                    ...
                    App.Vector(Lx_mm, 0, H1_mm + tf_mm_floor2)                    # 閉じる
                ]
            else:  # 内傾斜の場合
                # 通常の台形（屋根下端まで）
                points = [
            ...
            if window_ratio_2f > 0:
                ...
                # 窓の中心位置での壁のX座標を計算
                window_center_z = H1_mm + tf_mm_floor2 + H2_mm * 0.5
                ...
                window_box.translate(App.Vector(
                    ...
                    H1_mm + tf_mm_floor2 + H2_mm * window_bottom_margin  # 動的マージンを使用
                ))
                east_wall_2f = east_wall_2f.cut(window_box)
        else:
            # 傾斜なしの通常の壁
            east_wall_2f = Part.makeBox(tw_ext_mm, Ly_mm, H2_mm).translate(App.Vector(Lx_mm, 0, H1_mm + tf_mm_floor2))
            # 通常の窓開口処理
            if window_ratio_2f > 0:
                window_thickness = tw_ext_mm * 4  # 壁厚の4倍
                glass_wall = Part.makeBox(window_thickness, Ly_mm * window_ratio_2f, H2_mm * 0.7)
                glass_wall.translate(App.Vector(Lx_mm - tw_ext_mm * 2, Ly_mm * (1 - window_ratio_2f) / 2, H1_mm + tf_mm_floor2 + H2_mm * 0.15))
                east_wall_2f = east_wall_2f.cut(glass_wall)
        
        walls.append(east_wall_2f)
        
        # 西面壁（通常の垂直壁）
        west_wall_2f = Part.makeBox(tw_ext_mm, Ly_mm, H2_mm).translate(App.Vector(-tw_ext_mm, 0, H1_mm + tf_mm_floor2))
        ...
        if balcony_depth_mm > 0:
            ...
            balcony_door.translate(App.Vector(-tw_ext_mm - 10, door_y_position, H1_mm + tf_mm_floor2 + 100))  # 床から100mm上
        ...
        if abs(wall_tilt_angle) > 0.1:  # 傾斜がある場合（内外問わず）
            ...
            south_points = [
                App.Vector(0, -tw_ext_mm, H1_mm + tf_mm_floor2),                    # 左下
                App.Vector(Lx_mm, -tw_ext_mm, H1_mm + tf_mm_floor2),               # 右下
                App.Vector(Lx_mm + wall_offset_top, -tw_ext_mm, total_height_mm),  # 右上（傾斜に合わせる）
                App.Vector(0, -tw_ext_mm, total_height_mm),                 # 左上
                App.Vector(0, -tw_ext_mm, H1_mm + tf_mm_floor2)                    # 閉じる
            ]
            ...
            for i in range(4):
                ...
                window_z_position = H1_mm + tf_mm_floor2 + H2_mm * window_top_margin
                window_center_z = window_z_position + window_height * 0.5
                
                # 傾斜による窓のX位置のオフセット
                if abs(wall_tilt_angle) > 0.1:
                    x_offset_at_window = (window_center_z - (H1_mm + tf_mm_floor2)) * math.tan(tilt_rad)
                else:
            ...
            # 傾斜なしの通常の南面壁
            south_wall_2f = Part.makeBox(Lx_mm, tw_ext_mm, H2_mm).translate(App.Vector(0, -tw_ext_mm, H1_mm + tf_mm_floor2))
        ...
        if abs(wall_tilt_angle) > 0.1:  # 傾斜がある場合（内外問わず）
            ...
            north_points = [
                App.Vector(0, Ly_mm, H1_mm + tf_mm_floor2),                        # 左下
                App.Vector(Lx_mm, Ly_mm, H1_mm + tf_mm_floor2),                   # 右下
                App.Vector(Lx_mm + wall_offset_top, Ly_mm, total_height_mm),    # 右上（傾斜に合わせる）
                App.Vector(0, Ly_mm, total_height_mm),                      # 左上
                App.Vector(0, Ly_mm, H1_mm + tf_mm_floor2)                        # 閉じる
            ]
            ...
            for i in range(6):
                ...
                small_window.translate(App.Vector(
                    ...
                    H1_mm + tf_mm_floor2 + H2_mm * 0.35
                ))
                
                north_wall_2f = north_wall_2f.cut(small_window)
        else:
            # 傾斜なしの通常の北面壁
            north_wall_2f = Part.makeBox(Lx_mm, tw_ext_mm, H2_mm).translate(App.Vector(0, Ly_mm, H1_mm + tf_mm_floor2))
            for i in range(6):
                # 窓ボックスを十分大きく作成（確実に貫通させるため）
                window_thickness = tw_ext_mm * 4  # 壁厚の4倍
                # window_ratio_2fに基づいて窓サイズを調整
...
def create_external_stairs(Lx_mm, Ly_mm, H1_mm, H2_mm, tf_mm):
    ...
    for i in range(num_steps + 1): # 階段の開始と各段の終端
        ...
        # 2階フロアの上面までで制限
        point_z = min(point_z, H1_mm + tf_mm_floor2)
...
def calculate_economic_cost(building_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for part, ratio in parts_volume_ratio.items():
        ...
        if part == 'columns' and building_info.get('material_columns', 0) == 1:
            # 木製柱
            material = 'wood'
            labor_factor = 1.3  # 木造の施工コスト割増
        elif part == 'floors' and (building_info.get('material_floor1', 0) == 1 or building_info.get('material_floor2', 0) == 1):
            # 木製床（一部でも木材なら木材コストで計算）
            material = 'wood'
            labor_factor = 1.5
        elif part == 'roof' and building_info.get('material_roof', 0) == 1:
            # 木製屋根
            material = 'wood'
            labor_factor = 1.4
        elif part == 'walls' and building_info.get('material_walls', 0) == 1:
            # 木製壁
            material = 'wood'
            labor_factor = 1.2
        elif part == 'balcony' and building_info.get('material_balcony', 0) == 1:
            # 木製バルコニー
            material = 'wood'
            labor_factor = 1.6
        else:
        ...
        # 材料費
        material_cost = part_volume * MATERIAL_PROPERTIES[material]['cost_per_m3']
        # 労務費（材料費の50%をベースに）
...
def calculate_environmental_impact(building_info: Dict[str, Any], fem_results: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    for part, ratio in parts_volume_ratio.items():
        ...
        # CO2排出量
        co2_emission = part_volume * MATERIAL_PROPERTIES[material]['co2_per_m3']
