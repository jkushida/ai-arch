...
"""
building_fem_analysis_piloti_with_stairs_v9.py
...
def calculate_economic_cost(building_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 床面積計算
    Lx = building_info.get('span_length', 8.0)
    Ly = building_info.get('span_length', 8.0)
    ...
    # 6. 複雑性によるコスト増（既存のロジックを維持）
    complexity_factor = (
        1 + building_info.get('asymmetry_factor', 0) * 0.1 +
        1 + building_info.get('opening_complexity', 0) * 0.05 +
        1 + building_info.get('structural_irregularity', 0) * 0.15 +
        1 + building_info.get('has_cantilever', False) * 0.1 +
        1 + building_info.get('has_stairs', False) * 0.08
    )
...
def calculate_environmental_impact(building_info: Dict[str, Any], fem_results: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    建物の環境負荷（CO2排出量）を評価する（FEM結果活用版）
    """
    mass_kg = building_info.get('mass', 0.0)
    
    # 基本的なCO2排出量計算
    co2_per_kg_rc = 0.2  # kg-CO2/kg
    total_co2_emission_kg = mass_kg * co2_per_kg_rc
    ...
    if fem_results:
        ...
        if stress_utilization < 0.3:  # 30%未満
            optimization_potential = 0.3  # 30%削減可能
        elif stress_utilization < 0.5:  # 50%未満
            optimization_potential = 0.15  # 15%削減可能
        
        # 応力が不均一な場合、最適化余地あり
        if stress_uniformity < 0.5:
            optimization_potential += 0.1
    
    # 最適化後のCO2排出量
    optimized_co2_emission = total_co2_emission_kg * (1 - optimization_potential * 0.7)
    ...
    return {
        ...
        'material_efficiency_score': stress_utilization if fem_results else 0.5
    }
...
def calculate_comfort_score(fem_results: Dict[str, Any], building_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if avg_height >= 3.5:
        ...
        height_score = 8.0 + (avg_height - 3.0) * 4.0
    elif avg_height >= 2.4:
        height_score = 5.0 + (avg_height - 2.4) * 5.0
    else:
        height_score = avg_height / 2.4 * 5.0
    ...
    if avg_span >= 12:
        ...
        span_score = 7.0 + (avg_span - 8) * 0.75
    else:
        span_score = avg_span / 8 * 7.0
    ...
    if window_ratio >= 0.8:
        ...
        daylight_score = 8.0 + (window_ratio - 0.6) * 10.0
    elif window_ratio >= 0.3:
        daylight_score = 5.0 + (window_ratio - 0.3) * 10.0
    else:
        daylight_score = window_ratio / 0.3 * 5.0
    
    # 階高による眺望の良さ
    view_score = min(10.0, H1 / 3.0 * 5.0 + 5.0)  # 1階が高いほど眺望良好
    
    lighting_score = (daylight_score * 0.7 + view_score * 0.3)
    
    # ========== 3. ピロティによる開放感（20%） ==========
    piloti_score = 8.0  # 基本点
    ...
    # ========== 4. プライバシー・静粛性（15%） ==========
    # 2階居住による静粛性
    privacy_score = 7.0  # 基本点（1階がピロティで静か）
    ...
    if disp_ratio <= 0.5:
        ...
        structural_comfort = 10.0 - (disp_ratio - 0.5) * 10.0
    else:
        structural_comfort = max(0, 5.0 - (disp_ratio - 1.0) * 5.0)
    ...
    if building_info.get('has_balcony', False):
        ...
        if balcony_depth >= 2.0:
            balcony_bonus = 1.5  # 広いバルコニーは大きなボーナス
