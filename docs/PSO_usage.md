...
graph LR
    ...
    C --> B
    B --> D((JSON))
    D -->|Realtime| E[Monitor]
```

- **A: 設定ファイル** - `pso_config.py`（パラメータと目的関数の定義）
- **B: PSOループ** - `pso_algorithm.py`（最適化アルゴリズムの実行）
- **C: 粒子の評価** - `generate_building_fem_analyze.py`（建物生成とFEM解析）
- **D: JSONデータ** - `pso_realtime_data.json`（リアルタイムデータ）
- **E: 監視スクリプト** - `monitor_pso_mac.py` / `monitor_pso_win.py`（進捗可視化）
