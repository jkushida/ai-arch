...
このシステムは、建築設計の最適化プロセスを自動化することを目的として開発されました。従来、建築設計では構造安全性、コスト、環境負荷などの複数の要素を手動で調整しながら最適解を探す必要がありましたが、本モジュールはこのプロセスを完全に自動化します。21個の設計パラメータを入力として受け取り、3次元建物モデルを自動生成し、構造解析を実行した後、5つの観点から性能を定量的に評価します。
...
最初のステップでは、21個の設計変数（建物の寸法、材料、デザイン要素など）を受け取ります。次に、FreeCADのAPIを使用してこれらのパラメータから3次元建物モデルを自動生成します。生成されたモデルは、CalculiXソルバーに渡されてFEM構造解析が実行されます。最後に、解析結果と建物情報を基に、5つの性能指標（安全性、経済性、環境性、快適性、施工性）が計算されます。
...
本システムの特徴の一つは、建物の設計を21個のパラメータで完全に定義できる点です。これらのパラメータは、実際の建築設計で考慮される主要な要素を網羅しており、形状に関する14個のパラメータと材料選択に関する6個のパラメータ、そして基礎（常にコンクリート固定）で構成されています。
...
|---|---|:---:|---|
| **構造的複雑さ** |  |  |  |
| | カンチレバー（片持ち構造） | **-2.0点** | 特殊な支保工が必要 |
| | 外部階段 | **-1.5点** | 別途施工が必要 |
| | 開口部複雑度 | **-0.5点/単位** | 型枠の複雑化 |
| **形状の特殊性** |  |  |  |
| | 壁傾斜 | **-0.1点/度** | 傾斜型枠の施工難度 |
| | かまぼこ屋根（標準曲率） | **-0.5点** | 0.2 < roof_morph < 0.7 |
| | かまぼこ屋根（急曲率） | **-1.5点** | roof_morph ≥ 0.7 |
| | バルコニー深さ | **-0.1～-0.5点** | 深いほど施工困難 |
| **材料・寸法の特殊性** |  |  |  |
| | 過大断面 | **-1.0点** | 標準の1.5倍以上 |
| | 異種材料混在 | **-0.5点** | 施工管理の複雑化 |
| | アクセス困難な高所作業 | **-1.0点** | 安全対策の必要性 |
| **簡略化要因** |  |  |  |
| | ピロティ構造 | **+1.0点** | 型枠簡略化 |
| | 標準寸法の使用 | **+0.5点** | 規格材の活用可能 |
...
| 評価指標 | FEM解析結果の使用方法 | 具体的な利用項目 |
|---|---|---|
| **安全性** | 直接使用 | 最大応力、最大変位 |
| **経済性** | 間接使用 | 安全率による材料最適化の判断 |
| **環境性** | 間接使用 | 安全率による材料削減ポテンシャル |
| **快適性** | 直接使用 | 最大変位による構造的安心感 |
| **施工性** | 間接使用 | 応力集中による補強必要性の判断 |

FEM解析の結果は、単に構造安全性を評価するだけでなく、建物の経済性、環境性、快適性、施工性といった多面的な評価に活用されています。これにより、構造的に成立するだけでなく、実用的で最適な建築設計を導き出すことが可能となっています。
...
パラメトリック設計は、建物の形状を数値パラメータで制御する手法です。本システムでは21個のパラメータを使用して、無限に近い設計バリエーションを生成できます。
