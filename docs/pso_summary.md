...
1. **初期化**：すべての粒子の位置ベクトル $\boldsymbol{x}_i$ と速度ベクトル $\boldsymbol{v}_i$ を解空間内でランダムに設定する．各粒子は異なる位置から探索を開始する．

2. **速度の更新**：各粒子の速度 $\boldsymbol{v}_i$ を更新式に従って計算する．慣性項・認知項・社会項を合成し，次の移動方向と速さを決める．

3. **位置更新と評価**：更新後の速度を用いて位置 $\boldsymbol{x}_i$ を更新し，その位置で目的関数値 $f(\boldsymbol{x}_i)$ を計算する．

4. **pbestの更新**：粒子ごとに，現在の評価値が過去の個体最良値より良ければ，個体最良位置 $pbest_i$ を現在位置に更新する．

5. **gbestの更新**：全粒子の $pbest$ のうち最良のものを $gbest$ として更新する．

6. **終了判定**：最大ステップ数に達したか，または所定の収束基準を満たしたかを判定する．

7. **継続時のステップ更新**（No の場合）：ステップ数を更新 $t \leftarrow t+1$ し，**速度更新**へ戻ってループを継続する．

8. **最適解の出力**（Yes の場合）：$gbest$ を最適化問題の解として出力する．
